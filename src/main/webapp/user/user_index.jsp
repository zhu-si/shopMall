        ...
        <c:forEach items="${requestScope.list}" var="user">
          <tr>
            <th><input type="checkbox" class="chk" myid="${user.id}"></th>
					<th>${user.id}</th><th>${user.email}</th><th>${user.password}</th><th>${user.tel}</th>
					
					<c:if test="${user.level==0}"><th>白银</th></c:if>
					<c:if test="${user.level==1}"><th>黄金</th></c:if>
					<c:if test="${user.level==2}"><th>钻石</th></c:if>
					<c:if test="${user.level==3}"><th>至尊</th></c:if>
					<c:if test="${user.level==4}"><th>超级</th></c:if>
					<th>${user.amount}</th>
					<c:if test="${user.status==0}"><th>活跃</th></c:if>
					<c:if test="${user.status==1}"><th>潜水</th></c:if>
					<th>${user.comments}</th>
            
            <th class="td-manage">
              <a title="修改" onclick="javascript:openwinee(${user.id});" >
                <i class="layui-icon">&#xe642;</i>
              </a>
              <a title="删除" href="javascript:del(${user.id});"><i class="layui-icon">&#xe640;</i>
              </a>
            </th>
          </tr>
          </c:forEach>
